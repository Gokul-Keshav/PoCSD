...
class FSShell():
    ...
    def repair(self, server):
        ...
        try:
            ...
            print('Error: ' + str(server) + ' not a valid Integer')
            return -1
        
        if server >= fsconfig.NUM_SERVERS:
            print('Error: ' + server + ' server id not valid')
            return -1
        
        i, errorcode = self.RawBlocks.Repair(server)
        if i == -1:
            print("Error: " + errorcode)
            return -1
        return 0
